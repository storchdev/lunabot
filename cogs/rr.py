    ...
    async def submit(self, inter, button):
        if not (self.message or self.embed):
            await inter.response.send_message(
                "Please enter either a message or an embed.", ephemeral=True
            )
            return
    ...
    async def submit(self, inter, button):
        if self.limit is None:
            return await inter.response.send_message(
                "Please choose a limit first.", ephemeral=True
            )
...
class TimeModal(ui.Modal, title="Set minimum time since joining"):
    ...
    async def on_submit(self, inter):
        try:
            ...
            return await inter.response.send_message(
                "Please enter a valid time.", ephemeral=True
            )
...
class TimeDenyModal(ui.Modal, title="When user hasn't stayed long enough"):
    text = ui.TextInput(
        ...
        default="Use {time} for the remaining time thingy (DO NOT TYPE 'in' BEFORE IT)",
    )
...
class RR(commands.Cog, name="Reaction Roles"):
    ...
    async def addrr(self, ctx, *, channel: discord.TextChannel):
        embed = (
            discord.Embed(
                title="Make the message for reactions to go under", color=0xCAB7FF
            )
            .add_field(name="Message Text", value="None")
            .add_field(name="Embed", value="None")
        )
        ...
        while not joever:
            ...
            try:
                ...
                await ctx.send(
                    "Something went wrong while parsing that! Please try again."
                )

        view2 = RRView2(ctx, len(stuff))
        await ctx.send(
            "Choose a limit to the number of roles someone can get from this group:",
            view=view2,
        )
        await view2.wait()
        ...
        embed = (
            discord.Embed(title="Optional Settings", color=0xCAB7FF)
            .add_field(name="Role Requirement", value="None")
            .add_field(name="Time Requirement", value="None")
        )
        ...
        await self.bot.db.execute(
            ...
            json.dumps(stuff),
            int(view2.limit),
            view3.role.id if view3.role else None,
            view3.role_denymsg,
            view3.seconds,
            view3.time_denymsg,
        )
            ...
            if (
                ...
                if row["no_time_msg"] is not None:
                    await payload.member.send(
                        row["no_time_msg"].replace(
                            "{time}",
                            discord.utils.format_dt(
                                payload.member.joined_at
                                + datetime.timedelta(seconds=row["req_time"]),
                                "R",
                            ),
                        )
                    )
