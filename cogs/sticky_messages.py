...
class StickyMessages(commands.Cog):
    ...
    async def cog_load(self):
        ...
        for row in rows:
            self.sticky_messages[row["channel_id"]] = StickyMessage.from_db_row(
                self.bot, row
            )
    ...
    async def on_message(self, msg):
        ...
        sm = discord.utils.find(
            lambda sm: sm.channel == msg.channel, self.sticky_messages.values()
        )
    ...
    async def listsm(self, ctx):
        ...
        embed = discord.Embed(
            title="Sticky Messages", color=self.bot.DEFAULT_EMBED_COLOR
        )
