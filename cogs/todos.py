...
class Todo:
    def __init__(
        self, id, name, priority, completed, creator_id, time_created, time_completed
    ):
        self.id = id
        self.name = name
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if count > 0:
            await interaction.response.send_message(
                f'A todo with the name "{new_name}" already exists.', ephemeral=True
            )
            return
        ...
        await interaction.response.edit_message(
            embed=self.view.format_todo_list(), view=self.view
        )
...
class TodoPageSource(menus.ListPageSource):
    ...
    def get_filtered_sorted_todos(self):
        filtered_todos = [
            ...
            if self.show_completed or not todo.time_completed
        ]
        sorted_todos = sorted(
            filtered_todos,
            key=lambda todo: (
                getattr(todo, self.sort_by) is None,
                getattr(todo, self.sort_by),
            ),
        )  # sorts while pushing Nones to the end
        return sorted_todos
    ...
    async def format_page(self, menu, todos):
        ...
        return discord.Embed(
            color=self.bot.DEFAULT_EMBED_COLOR, description="\n".join(lines)
        )
...
class TodoListView(ViewMenuPages):
    ...
    @discord.ui.button(
        label="Show Completed", style=discord.ButtonStyle.secondary, row=1
    )
    async def toggle_show_completed_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        await self.handle_toggle_show_completed(interaction)

    @discord.ui.select(placeholder="Sort by...", row=2)
    async def sort_by_select(
...
class TodoEditView(View):
    ...
    def update_todo_list(self):
        ...
        todo_index = next(
            (i for i, todo in enumerate(todos) if todo.name == self.todo_name), None
        )
    ...
    async def handle_move_up(self, interaction: discord.Interaction):
        ...
        await self.update_todo_priority(
            interaction, new_priority=max(1, current_priority - 1)
        )
    ...
    async def handle_move_down(self, interaction: discord.Interaction):
        ...
        current_priority = self.todos_to_display[self.todo_index].priority
        await self.update_todo_priority(
            interaction, new_priority=min(max_priority, current_priority + 1)
        )
    ...
    async def update_todo_priority(
        self, interaction: discord.Interaction, new_priority: int
    ):
        current_todo = self.todos_to_display[self.todo_index]
        ...
        self.update_todo_list()
        await interaction.response.edit_message(
            embed=self.format_todo_list(), view=self
        )

    @discord.ui.button(label="⤒", style=discord.ButtonStyle.primary, row=0)
    async def move_to_top_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
    ...
    @discord.ui.button(label="⤓", style=discord.ButtonStyle.primary, row=0)
    async def move_to_bottom_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        await self.handle_move_to_bottom(interaction)

    @discord.ui.button(
        emoji="\U0000270f\U0000fe0f", style=discord.ButtonStyle.secondary, row=0
    )
    ...
    async def submit_button(
        ...
        await interaction.response.edit_message(
            content="Changes submitted.", view=None, embed=None
        )
        self.stop()
    ...
    async def cancel_button(
        ...
        await interaction.response.edit_message(
            content="Edit canceled.", view=None, embed=None
        )
        self.stop()
...
class TodoCog(commands.Cog, name="Todos"):
    ...
    async def add_todo_command(self, ctx, *, name: str):
        ...
        current_todo_count = await self.bot.db.fetchval(
    ...
    async def list_todos_command(self, ctx):
        ...
        source = TodoPageSource(
            self.bot, todos, show_completed=False, sort_by="priority"
        )
    ...
    async def edit_todo_command(self, ctx, *, name: str):
        ...
        todos = await self.bot.db.fetch(
            "SELECT * FROM todos WHERE completed = FALSE ORDER BY priority"
        )
        source = TodoPageSource(
            self.bot, todos, show_completed=True, sort_by="priority"
        )
