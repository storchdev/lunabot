...
from .utils import View, staff_only, admin_only
    ...
    async def create_ticket(self):
        ...
        query = """INSERT INTO
                   ...
                   VALUES
                       ($1, $2, $3, $4)
                """
        await self.bot.db.execute(query, ticket.id, ticket.thread.id, ticket.opener.id, ticket.timestamp.timestamp())
...
class TicketCog(commands.Cog, name='Tickets v2', description='thread tickets'):
    ...
    async def transcript(self, ctx, ticket_id: int):
        ...
        if file is None:
            helpdesk = self.bot.get_var_channel('helpdesk') 
            await ctx.send(f'No transcript found for that ticket. Check the threads in {helpdesk.mention}?')
            return 
        await ctx.send(file=file)

    @commands.command()
    @admin_only()
    async def sendticketembed(self, ctx, channel: discord.TextChannel, *, embed_name: str):
        embed_name = embed_name.lower()
        if embed_name not in self.bot.embeds:
            await ctx.send(f'Embed `{embed_name}` not found!')
            return

        button = ui.Button(label='❀﹒﹒Click me!﹒﹒❀', emoji="<a:Lumi_heart_float:924048498477891614>", style=discord.ButtonStyle.blurple)
