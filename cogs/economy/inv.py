...
class InvSearchModal(ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if results:
            ...
            results_source = InvResultsPageSource(
                self.parent_view.ctx, results, "time_acquired"
            )
...
class InvMainPageSource(menus.ListPageSource):
    ...
    def get_sorted_entries(self):
        if self.sort_by == "time_acquired":
            ...
            return sorted(
                self.entries, key=lambda e: e["item"].sell_price, reverse=True
            )
