...
from .utils import View, staff_only
...
class AffirmationView(View):
    ...
    async def affirm(self, interaction, button):
        ...
        repls = {"number": self.confession_id, "usermention": f"<@{self.user_id}>"}
        ...
        # temporary fix
            ...
            async for msg in channel.history(after=after, before=before, limit=None):
                if (
                    ...
                    and msg.embeds
                    and msg.embeds[0].description.startswith(
                        f"> \u207a <a:ML_heart_point:917958056409706497>\ufe52**Confession #{number}"
                    )
                ):
                    await msg.delete()
                    found = True
