...
class HelpSelectMenu(discord.ui.Select["HelpMenu"]):
    def __init__(
        self, entries: dict[commands.Cog, list[commands.Command]], bot, number: int = 0
    ):
        if number == 0:
...
class FrontPageSource(menus.PageSource):
    ...
    def format_page(self, menu, page: Any):
        embed = discord.Embed(
            title="❀ㆍㆍBot Help﹗⁺ ₍ <a:LCD_Nikki_yay:1127849118992171058> ₎",
            colour=0xCAB7FF,
        )
        embed.description = """** **                          ‧  ╴‧  ╴‧  ╴‧
...
> <:LC_reply_F2U:1081275742765195355> You __cannot__ add me to your own servers! I am solely here to serve Moonlit Lilac servers. Please do not ask, nor attempt to add me elsewhere.
                           ‧  ╴‧  ╴‧  ╴‧"""
        embed.set_footer(
            text="⁺﹒This help command was borrowed from Danny's bot R. Danny﹒⁺"
        )
        if self.index == 1:
            entries = (
