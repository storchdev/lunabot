...
#
# Modified version of the PaginatedHelpCommand and helper classes, from RoboDanny, source/credits:
...
from cogs.utils import RoboPages
...
class GroupHelpPageSource(menus.ListPageSource):
    ...
    async def format_page(self, menu: RoboPages, commands: list[commands.Command]):
        embed = discord.Embed(
            title=self.title, description=self.description, colour=0xCAB7FF
        )
...
class HelpSelectMenu(discord.ui.Select["HelpMenu"]):
    def __init__(
        self,
        entries: dict[commands.Cog, list[commands.Command]],
        bot: "LunaBot",
        number: int = 0,
    ):
        if number == 0:
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        value = self.values[0]
        if value == "__index":
            base_embed = self.bot.get_embed("help")
            await self.view.rebind(FrontPageSource(base_embed), interaction)
        else:
...
class FrontPageSource(menus.PageSource):
    ...
    def format_page(self, menu, page: Any):
        if self.base_embed is None:
            embed = discord.Embed(title="Placeholder Help Embed")
        else:
...
class PaginatedHelpCommand(commands.HelpCommand):
    ...
    async def send_bot_help(self, mapping):
        ...
        base_embed = self.context.bot.get_embed("help")
        menu = HelpMenu(FrontPageSource(base_embed), ctx=self.context)
