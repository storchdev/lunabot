    ...
    async def interaction_check(self, inter):
        ...
        if inter.user.id == self.opener_id:
            await inter.response.send_message(
                "Sorry, only staff can close tickets.", ephemeral=True
            )
            return False
        return True
...
class TicketTypeMenu(View):
    ...
    async def create_ticket(self):
        ...
        view = CloseView(
            self.bot, ticket.id, ticket.channel, ticket.opener.id, ticket.timestamp
        )
        ...
        await archive.send(
            f"{pings}: get ready to rizz up {self.owner.mention} in {channel.mention} on skibidi"
        )
    ...
    async def create_channel(self, ticket: Ticket):
        ...
        overwrites = {
            self.owner.guild.default_role: discord.PermissionOverwrite(
                view_channel=False
            ),
        ...
        ticket.channel = await archive.category.create_text_channel(
            name=f"ticket-{ticket_id}", overwrites=overwrites
        )
...
class TicketCog(
    commands.Cog,
    name="Tickets",
    description="a few sketchy admin-only sketchy commands",
):
    def __init__(self, bot):
        self.bot: "LunaBot" = bot
    ...
    async def sendembed(self, ctx, channel: discord.TextChannel):
        for view in self.bot.persistent_views:
            if view.channel_id == channel.id:
                ...
                return
        await ctx.send(
            "i dont have a ticket prepared for that channel yet, pls contact storch"
        )
        ...
        for view in self.bot.persistent_views:
            if view.required_role_id == role.id:
                try:
                    ...
                    await ctx.send(
                        "I couldnt give that member the role, make sure im not being permission hiearchyd"
                    )
                    return
                ...
                await self.bot.db.execute(
                    ...
                    view.custom_id,
                    member.id,
                    int(time.time() + view.cooldown),
                    "initial wait",
                )

                channel = self.bot.get_channel(view.channel_id)
                md = discord.utils.format_dt(
                    discord.utils.utcnow() + timedelta(seconds=view.cooldown), "d"
                )
                await ctx.send(
                    f"I gave {member.mention} the role; they can apply in {channel.mention} on {md}"
                )
                return
