...
class LunaScript(TextEmbed):
    def __init__(self, msgble, text=None, embed=None, **kwargs):
        ...
        if isinstance(msgble, commands.Context):
            ...
            self.script_ctx = ScriptContext(
                kwargs.pop("bot"), args=args, channel=channel, **kwargs
            )
            self.msgble = msgble
    ...
    async def reply(self, msg):
        try:
            ...
            return await msg.reply(
                f"An error occurred while parsing the LunaScript: `{e}`"
            )
    ...
    async def send(self, *args, **kwargs):
        try:
            if self.text is None:
                ...
                text = await self.parser.parse(self.text)
            return await self.msgble.send(
                text, embed=await self.transform_embed(), *args, **kwargs
            )
        except LunaScriptError as e:
            return await self.msgble.send(
                f"An error occurred while parsing the LunaScript: `{e}`"
    ...
    async def transform_embed(self):
        ...
        if self.embed.author.name is not None:
            self.embed.set_author(
                name=await self.parser.parse(self.embed.author.name),
                icon_url=self.embed.author.icon_url,
            )
        if self.embed.footer.text is not None:
            self.embed.set_footer(
                text=await self.parser.parse(self.embed.footer.text),
...
class LunaScriptParser:
    ...
    async def parse(self, text):
        return await self.script_ctx.bot.loop.run_in_executor(
            None, self.parse_sync, text
        )
            ...
            while i < len(string):
                ...
                if string[i] == "[":
                    ...
                    try:
                        ...
                        raise InvalidFunctionArgs(
                            f"Invalid arguments for {funcname}: {inside}"
                        )
                    ...
                    try:
                        ...
                        raise InvalidMathExpression(
                            f"Invalid math expression: {inside}"
                        )
                    newstr.extend([char for char in str(repl)])
                    i += j - i + 1
                elif string[i] == "<":
