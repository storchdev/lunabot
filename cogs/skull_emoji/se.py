...
# START_TIME = datetime(2023, 10, 15, 13).astimezone(ZoneInfo('America/Chicago'))
# OPTION3_TIME = 30
# OPTION4_TIME = 30
# OPTION5_TIME = 30
# OPTION5_CD = 10
...
class RedeemView(ui.View):
    def __init__(self, ctx, choices, powerups):
        ...
        for i, choice in enumerate(choices):
            ...
            btn = ui.Button(
                label=str(i),
                custom_id=str(powerups.index(choice)),
...
class Team:
    ...
    async def option5(self):
        for player in self.players:
            ...
            await player.apply_powerup(
                CooldownReducer(OPTION5_CD, time.time(), time.time() + OPTION5_TIME),
                log=log,
            )
...
class Player:
    ...
    async def apply_powerup(self, powerup, *, log=False):
        ...
        await self.bot.db.execute(
            query, self.member.id, powerup.name, powerup.n, powerup.start, powerup.end
        )
    ...
    async def log_msg(self):
        ...
        query = "insert into se_log (team, user_id, type, gain, time) values (?, ?, ?, ?, ?)"
        await self.bot.db.execute(
            query, self.team.name, self.member.id, "all_msg", 1, int(time.time())
        )
    ...
    async def add_points(self, points, reason, multi=True):
        ...
        query = "insert into se_log (team, user_id, type, gain, time) values (?, ?, ?, ?, ?)"
        await self.bot.db.execute(
            query, self.team.name, self.member.id, reason, gain, int(time.time())
        )
    ...
    async def on_500(self):
        ...
        ls = LunaScript.from_layout(
            self.team.channel, layout, args=args, member=self.member
        )
        await ls.send()
...
class ServerEvent(commands.Cog):
    ...
    async def cog_load(self):
        ...
        channels = {"bunny": 1158930467337293905, "kitty": 1158931468664446986}
        nicks = {
        ...
        for team_name, member_ids in playerdict.items():
            for member_id in member_ids:
                ...
                if team_name not in self.teams:
                    ...
                    saved_powerups = [
                        row["option"]
                        for row in await self.bot.db.fetch(query, team_name)
                    ]
                    self.teams[team_name] = Team(
                        team_name,
                        [],
                        self.bot.get_channel(channels[team_name]),
                        redeems,
                        saved_powerups,
                    )
                ...
                points = await self.bot.db.fetchval(
                    query, *self.non_point_types, member.id
                )
                if points is None:
                    points = 0
                ...
                player = Player(
                    self.bot, team, member, nicks[member.id], points, msgs, powerups
                )
    ...
    async def trivia(self, player, channel, steal=False):
        ...
        def check(r, u):
            return (
                r.message == msg and u.id == player.member.id and str(r.emoji) in emojis
            )
        ...
        if emojis[str(reaction.emoji)] == choices.index(a):
            if not steal:
                ...
                await channel.send(
                    f"You got the answer right! {player.nick} earned **{points}** points."
                )
            else:
                await player.team.opp.captain.remove_points(points, "steal_trivia")
                await player.add_points(points, "trivia")
                await channel.send(
                    f"You got the answer right! {player.nick} stole **{points}** points from the other team."
                )
        else:
            await channel.send(
                f"{player.nick} got the answer wrong! The correct answer was **{a}**.",
                delete_after=10,
            )
    ...
    async def on_message(self, msg):
        ...
        if self.msg_counter >= self.msgs_needed:
            ...
            if random.choice([True, False]):
                ...
                ls = LunaScript.from_layout(
                    msg.channel, layout, args={"powerupname": powerup_name}
                )
                spawn = await ls.send()
                await spawn.add_reaction(
                ...
                if powerup_i == 0:
                    ...
                    await player.apply_powerup(
                        CooldownReducer(
                            INDIV_REDUCED_CD,
                            time.time(),
                            time.time() + INDIV_REDUCED_CD_TIME,
                        ),
                        log=True,
                    )
                    ...
                    await player.apply_powerup(
                        Multiplier(2, time.time(), time.time() + INDIV_DOUBLE_TIME),
                        log=True,
                    )
                    ...
                    for other in player.team.players:
                        if other != player:
                            await other.apply_powerup(
                                Multiplier(
                                    2, time.time(), time.time() + TEAM_DOUBLE_TIME
                                )
                            )
                    ...
                    await player.apply_powerup(
                        Multiplier(3, time.time(), time.time() + INDIV_TRIPLE_TIME),
                        log=True,
                    )
                    ...
                    for other in player.team.players:
                        if other != player:
                            await other.apply_powerup(
                                Multiplier(
                                    3, time.time(), time.time() + TEAM_TRIPLE_TIME
                                )
                            )
                ...
                if layout is not None:
                    ...
                    ls = LunaScript.from_layout(
                        msg.channel, layout, args=args, member=player.member
                    )
                    await ls.send()
    ...
    async def usepowerup(self, ctx):
        ...
        for i, powerup_i in enumerate(powerups):
            embed.add_field(
                name=f"{i + 1}", value=self.powerups_1k[powerup_i], inline=False
            )
        ...
        if powerup == 0:
            ...
            await ctx.send(
                f"Your captain used a powerup that claimed **{n}** points for your team!"
            )
        elif powerup == 1:
            n = await team.option2()
            await ctx.send(
                f"Your captain used a powerup that stole **{n}** points from the other team!"
            )
        elif powerup == 2:
            await team.option3()
            await ctx.send(
                "Your captain used a powerup that **doubled** all incoming points for __30 minutes__!"
            )
        elif powerup == 3:
            await team.option4()
            await ctx.send(
                "Your captain used a powerup that **tripled** all incoming points for __20 minutes__!"
            )
        else:
            await team.option5()
            await ctx.send(
                "Your captain used a powerup that **reduced the cooldown** for messages to __1 minute__ for __25 minutes__!"
            )
        ...
        for team in self.teams:
            ...
            for player in self.teams[team].players:
                ...
                for powerup in player.powerups:
                    end = discord.utils.format_dt(
                        datetime.datetime.fromtimestamp(powerup.end), "R"
                    )
            ...
            if len(poweruplst) == 0:
                continue
            embed.add_field(
                name=f"Team {team.capitalize()}", value="\n\n".join(poweruplst)
            )
        await ctx.send(embed=embed)
        ...
        if flags.stat in {"msg", "messages", "message", "msgs"}:
            ...
            for team in teams:
                ...
                rows = await self.bot.db.fetch(
                    query, team.name, "all_msg", int(end.timestamp())
                )
            ...
            for team in teams:
                ...
                rows = await self.bot.db.fetch(
                    query, team.name, *self.non_point_types, int(end.timestamp())
                )
                rows_list.append((team, rows))
            data = data_from_rows(rows_list)
            file = await plot_data(self.bot, data)
            ...
            for team in teams:
                ...
                row = await self.bot.db.fetchrow(
                    query, team.name, *types, int(end.timestamp())
                )
            ...
            rows_list = []
            types = ["welc_bonus", "500_bonus", "topup_bonus", "steal_bonus"]
            placeholders = ",".join(["?"] * len(types))
            ...
            for team in teams:
                ...
                rows = await self.bot.db.fetch(
                    query, team.name, "trivia", int(end.timestamp())
                )
            ...
            for team in teams:
                ...
                rows = await self.bot.db.fetch(
                    query, team.name, "stolen", "steal_trivia", int(end.timestamp())
                )
            ...
            for team in teams:
                ...
                query = "select user_id, gain, time from se_log where team = ? and type = ? and time < ?"
                rows = await self.bot.db.fetch(
                    query, team.name, "welc_bonus", int(end.timestamp())
                )
