...
def hit5k():
    ...
    while len(dates) < 3:
        date = random_date(
            year_range=(2023, 2023), month_range=(8, 9), day_range=(1, 15)
        )
...
def animal():
    choices = ["Dog", "Penguin", "Shark", "Horse", "Dolphin", "Owl", "Gorilla"]
...
def musicgenre():
    choices = ["Punk Rock", "Hip-Hop", "Techno", "Classical", "K-Pop", "Fusion Jazz"]
    return random.sample(choices, 3)


def animal2():
    choices = ["Fox", "Lamb", "Goat", "Chipmunk", "Turtle", "Pig", "Owl"]
    return random.sample(choices, 3)
...
def hit690():
    ...
    while len(dates) < 3:
        date = random_date(
            year_range=(2022, 2022), month_range=(4, 4), day_range=(1, 30)
        )
...
questions = [
    ...
    (
        "What is Luna's least favorite part of owning a server",
        "Moderation",
        random_task,
    ),
    ...
    (
        'Who was involved with the magna "fun" laude in the gc Lumi Chaos?? **Hint: type Lumi Chaos to see the video**',
        "Luna, Nemi, Sunny, Molly, Vincent",
        funpeople,
    ),
    (
        'What is the origin of "Jonas"?? **Hint: type "Jonas" in chat**',
        "YAGPDB topic command",
        jonas,
    ),
    ...
    (
        "Who made these Luna emotes?? <:LC_Luna_woah_YumBun_NF2U:1118227655116988426> <:LC_Luna_peek_YumBun_NF2U:1118292331192393738> <:LC_Luna_rich_YumBun_NF2U:1118392831317385248>",
        "YumBun",
        notyumbum,
    ),
    (
        "What was the first thing Luna did when she created the server??",
        "Spam ping Nemi with the sticker welcome",
        firstthing,
    ),
    ("When did Josh first join Lumi Corp??", "11/14/21", josh),
    ("Who is the mod who has been with us the longest??", "Soy Sauce", longestmod),
    (
        "Why did Fwogiie get admin??",
        "Luna gave her the role so she could try to get the December 2021 server icon to upload",
        fwoggieadmin,
    ),
    (
        "Who made the server icon for December 2021?? Hint: it's the very first pinned message in chat",
        "Fwogiie",
        dec2021icon,
    ),
    ...
    (
        "Who were the 3 head admins before the role got merged with the admin role??",
        "Fwogiie, Josh, Levi",
        headadmins,
    ),
