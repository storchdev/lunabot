...
class ServerBattle(commands.Cog):
        ...
        for u, msgs in self.d.items():
            if guild.get_member(u.id) is None:
                for msg in msgs:
                    ...
                    logging.info(
                        f"Removed reaction from {u.name} ({u.id}) on {msg.jump_url}"
                    )
                log.append(
                    f"Removed reaction from {u.mention} ({u.name}) on {', '.join(m.jump_url for m in msgs)} due to leaving"
                )
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if payload.member in self.d:
            ...
            priv = self.bot.get_var_channel("private")
            await priv.send(
                f"removed vote by {payload.member.mention} on {msg.jump_url}"
            )
        else:
