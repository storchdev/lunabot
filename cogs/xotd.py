...
class XoTD(commands.Cog):
    ...
    async def cog_load(self):
        ...
        questions = await self.bot.db.fetchval(
            "SELECT items FROM queues WHERE name = $1", "qotd"
        )
        self.questions = json.loads(questions)
        songs = await self.bot.db.fetchval(
            "SELECT items FROM queues WHERE name = $1", "sotd"
        )
    ...
    async def post_qotd(self):
        ...
        msg = await layout.send(
            channel, repls={"number": number, "question": question["question"]}
        )
    ...
    async def add_qotd(self, ctx, *, question: str):
        ...
        await ctx.send(
            f"Your question was added to queue! It will be posted {md}. Note that questions are reviewed, and those that fail to follow the server rules will be removed.",
            ephemeral=True,
        )
    ...
    async def add_sotd(self, ctx, *, data: str):
        ...
        spotify_url = data[2]
        if not re.match(
            r"https://open\.spotify\.com/track/[A-Za-z0-9]{22}(\?si=[A-Za-z0-9_-]+)?",
            spotify_url,
        ):
