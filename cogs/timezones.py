...
class TimezoneCog(commands.Cog):
    DEFAULT_POPULAR_TIMEZONE_IDS = (
        'usnyc', 'uslax', 'uschi', 'usden', 'inccu', 'trist', 'rumow', 'gblon',
        'frpar', 'esmad', 'deber', 'grath', 'uaiev', 'itrom', 'nlams', 'plwaw',
        'cator', 'aubne', 'ausyd', 'brsao', 'jptyo', 'cnsha'
    )
        ...
        async with self.bot.session.get(
            ...
            for entry in entries.values():
                if entry['preferred']:
                    ...
                    if preferred_entry:
                        self._timezone_aliases[entry['description']] = preferred_entry['aliases'][0]
                else:
                    self._timezone_aliases[entry['description']] = entry['aliases'][0]

    @commands.hybrid_command(name="set-timezone", aliases=["settimezone", "settz", "set-tz"])
    ...
    async def set_timezone(self, ctx, *, timezone: str):
        ...
        await ctx.send(f"Timezone successfully set to `{match}` (**{selected_timezone}**)!")

    @commands.hybrid_command(name="get-timezone", aliases=["gettimezone", "gettz", "get-tz"])
    ...
    async def get_timezone(self, ctx, *, member: discord.Member = None):
        ...
        if other:
            await ctx.send(f"{member.display_name}'s timezone is set to `{tz}`.\nIt is {current_time}.")
        else:
