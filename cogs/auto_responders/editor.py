...
from cogs.utils import Cooldown, Layout, View

from .auto_responder import AutoResponder, AutoResponderAction
from .modals import (
    ChooseLayoutModal,
    CooldownModal,
    CreateLayoutModal,
...
class AutoResponderEditor(View):
    ...
    def embed(self):
        ...
        if self.cooldown:
            ...
            embed.add_field(
                name=f"Cooldown ({typev})",
                value=f"{self.cooldown.rate} use{s} per {self.cooldown.per}s",
                inline=False,
            )
    ...
    @ui.select(
        ...
        options=[
            ...
            discord.SelectOption(
                label="trigger is a regular expression", value="regex"
            ),
        ],
        row=1,
    )
...
class AddActionView(View):
    ...
    @ui.select(
        ...
        options=[
            ...
            discord.SelectOption(
                label="Delete Trigger", value="delete_trigger_message"
            ),
    ...
    async def set_action(self, interaction, select):
        ...
        if action_type == "send_message":
            ...
            await interaction.response.edit_message(
                content="Add reactions to this message, then press **Submit**",
                view=view,
            )
            ...
            for action in self.parent_view.actions:
                if action.type == "delete_trigger_message":
                    await interaction.response.send_message(
                        "You can only have one delete trigger message action!",
                        ephemeral=True,
                    )
                    return
            self.parent_view.actions.append(
                AutoResponderAction(self.bot, "delete_trigger_message")
            )
            self.parent_view.update()
            await interaction.response.edit_message(
                view=self.parent_view, embed=self.parent_view.embed
            )
...
class SendMessageEditor(View):
    ...
    def add_user_select(self):
        select = ui.UserSelect(
            row=1, placeholder="Select user (nothing = same)", min_values=0
        )
        ...
        async def callback(interaction):
            ...
            self.update()
            await interaction.response.edit_message(
                view=self, content=self.layout.content, embeds=self.layout.embeds
            )
    ...
    def add_channel_select(self):
        select = ui.ChannelSelect(
            row=1, placeholder="Select channel (nothing = same)", min_values=0
        )
...
class AddRolesView(View):
    ...
    async def select_roles(self, interaction, select):
        action = AutoResponderAction(
            self.bot, "add_roles", roles=[r.id for r in select.values]
        )
        self.original_view.actions.append(action)
        await interaction.response.edit_message(
            view=self.original_view, embed=self.original_view.embed
...
class RemoveRolesView(View):
    ...
    async def select_roles(self, interaction, select):
        action = AutoResponderAction(
            self.bot, "remove_roles", roles=[r.id for r in select.values]
        )
        self.original_view.actions.append(action)
        await interaction.response.edit_message(
            view=self.original_view, embed=self.original_view.embed
...
class AddReactionView(View):
    ...
    async def submit(self, interaction, button):
        ...
        if len(emojis) == 0:
            await interaction.response.send_message(
                "No reactions found on the message!", ephemeral=True
            )
            return
...
class RestrictionsView(View):
    ...
    @ui.select(
        options=[
            discord.SelectOption(
                label="Blacklist channels", value="blacklisted_channels"
            ),
            discord.SelectOption(
                label="Whitelist channels", value="whitelisted_channels"
            ),
...
class UserSelect(ui.UserSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        self.view.parent_view.restrictions[self.view.setting] = [
            obj.id for obj in self.values
        ]
        await interaction.response.edit_message(
            view=self.view.parent_view, embed=self.view.original_view.embed
        )
