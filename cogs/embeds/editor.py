...
#
# Modified version of the meta/views/embed/__init__.py file, from Duck Hideout Manager Bot, source/credits:
# https://github.com/DuckBot-Discord/duck-hideout-manager-bot/tree/main/cogs/meta/views/embed/__init__.py
...
class UndoView(utils.View):
    ...
    async def undo(
        self, interaction: BotInteraction, button: discord.ui.Button[UndoView]
    ):
        self.stop()
        self.parent.cancelled = False
        await interaction.channel.send(
            view=self.parent, embed=self.parent.current_embed
        )  # type: ignore
...
class FieldSelectorView(utils.View):
    ...
    def update_options(self):
        ...
        for i, field in enumerate(self.parent.embed.fields):
            self.pick_field.add_option(
                label=f"{i + 1}) {(field.name or '')[0:95]}", value=str(i)
            )
    ...
    async def actual_logic(
        self, interaction: BotInteraction, select: discord.ui.Select[Self]
    ) -> None:
...
class SendToView(utils.View):
    ...
    async def pick_a_channel(
        self,
        interaction: discord.Interaction,
        select: discord.ui.ChannelSelect[SendToView],
    ):
        await interaction.response.defer(ephemeral=True)
        channel = select.values[0]
        if not isinstance(interaction.user, discord.Member) or not interaction.guild:
            return await interaction.followup.send(
                "for some reason, discord thinks you are not a member of this server...",
                ephemeral=True,
            )
        channel = interaction.guild.get_channel_or_thread(channel.id)
        if not isinstance(channel, discord.abc.Messageable):
            return await interaction.followup.send(
                "That channel does not exist... somehow.", ephemeral=True
            )
        if not channel.permissions_for(interaction.user).send_messages:
            return await interaction.followup.send(
                "You cannot send messages in that channel.", ephemeral=True
            )
    ...
    async def stop_pages(
        self, interaction: discord.Interaction, button: discord.ui.Button[SendToView]
    ):
...
class EmbedEditor(utils.View):
    ...
    def shorten(_embed: discord.Embed):
        ...
        if len(embed) > 6000 and embed.description:
            embed.description = embed.description[
                : (len(embed.description) - (len(embed) - 6000))
            ]
        return embed
    ...
    def add_items(self):
        ...
        self.add_item(
            discord.ui.Button(
                row=0, label="General →", disabled=True, style=ButtonStyle.gray
            )
        )
        self.add_item(
            EditWithModalButton(
                EditEmbedModal, label="Set Main Properties", style=ButtonStyle.blurple
            )
        )
        self.add_item(
            EditWithModalButton(
                EditAuthorModal, row=0, label="Set Author", style=ButtonStyle.blurple
            )
        )
        self.add_item(
            EditWithModalButton(
                EditFooterModal, row=0, label="Set Footer", style=ButtonStyle.blurple
            )
        )
        # Row 2
        self.add_item(
            discord.ui.Button(
                row=1, label="Fields →", disabled=True, style=ButtonStyle.gray
            )
        )
        self.add_fields = EditWithModalButton(
            AddFieldModal, row=1, label="Add", style=ButtonStyle.green
        )
        ...
        # Row 4
        self.character_count: discord.ui.Button[Self] = discord.ui.Button(
            row=3, label="0/6,000 Characters", disabled=True
        )
        self.add_item(self.character_count)
        self.fields_count: discord.ui.Button[Self] = discord.ui.Button(
            row=3, label="0/25 Fields", disabled=True
        )
    ...
    async def send(self, interaction: BotInteraction, button: discord.ui.Button[Self]):
        if not self.embed:
            return await interaction.response.send_message(
                "Your embed is empty!", ephemeral=True
            )
