...
async def setup(bot):
    ...
    for ar in old_ars:
        ...
        if "autoemojis" in data:
            actions = json.dumps(
                [{"type": "add_reactions", "kwargs": {"emojis": data["autoemojis"]}}],
                indent=4,
            )
        else:
            actions = [
            ...
            if data.get("delete_trigger"):
                actions.append({"type": "delete_trigger_message", "kwargs": {}})

            if data.get("give_roles"):
                actions.append(
                    {"type": "add_roles", "kwargs": {"roles": data["give_roles"]}}
                )

            if data.get("remove_roles"):
                actions.append(
                    {"type": "remove_roles", "kwargs": {"roles": data["remove_roles"]}}
                )
        ...
        await db.execute(
            ...
            actions,
            json.dumps(restrictions, indent=4),
            cooldown,
            496225545529327616,
        )
    ...
    for sm in old_sms:
        ...
        text = sm["text"]
        embed = sm["embed"]
        layout = {"name": None, "content": text, "embeds": [embed] if embed else []}
        ...
        query = """INSERT INTO
                ...
                """
        await db.execute(
            query, channel.id, json.dumps(layout, indent=4), last_message_id
        )
